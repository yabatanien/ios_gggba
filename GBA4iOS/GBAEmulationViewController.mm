...
- (void)viewDidLoad
{
    ...
    {
        mAVCaptureSession = [AVCaptureSession new];
        mCameraDevice = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
        ...
        NSError *error = nil;
        mCameraInput = [[AVCaptureDeviceInput alloc] initWithDevice:mCameraDevice error:&error];
        
        //入力をセッションに追加
        if ([mAVCaptureSession canAddInput:mCameraInput]) {
            [mAVCaptureSession addInput:mCameraInput];
            [mAVCaptureSession beginConfiguration];
            //セッションの設定
            mAVCaptureSession.sessionPreset = AVCaptureSessionPresetPhoto;
            [mAVCaptureSession commitConfiguration];
        } else {
            NSLog(@"Error: %@", error);
        }
        // キャプチャーセッションから入力のプレビュー表示を作成
        AVCaptureVideoPreviewLayer *previewLayer = [[AVCaptureVideoPreviewLayer alloc] initWithSession:mAVCaptureSession];
        
        CGSize screenSize = [UIScreen mainScreen].bounds.size;
        mCameraView = [[UIView alloc] initWithFrame:CGRectMake((screenSize.width - 200) / 2, 75, 200, 300)];
...
//        previewLayer.videoGravity = AVLayerVideoGravityResizeAspectFill;  // <-- アスペクト比を変えない。レイヤーからはみ出した部分は隠す。
        ...
        //UIViewの境界矩形外は描写しない
        mCameraView.layer.masksToBounds = YES;
...
-(void)processMainThread:(void(^)(void))func{
...
//                CGSize maxScreenSize = self.view.bounds.size;
    ...
    
    {
        ...
        CGFloat partBottom = (inStackView.frame.origin.y + inStackView.frame.size.height) * 0.85;
        
        mLabelComment = [[UILabel alloc] initWithFrame:CGRectMake(0, partBottom, [UIScreen mainScreen].bounds.size.width, 120)];
        [mLabelComment setBackgroundColor:[UIColor blackColor]];
        [mLabelComment setNumberOfLines:0];
        [mLabelComment setTextColor:[UIColor cyanColor]];
...
-(void)loginWithInfo:(NicoLoginInfo)info complete:(void(^)(BOOL success, NSString *reasion)) onComplete{
    ...
    NSString *password = (__bridge NSString *)CFURLCreateStringByAddingPercentEscapes(
                                                                                      NULL,
                                                                                      (CFStringRef)info.password,
                                                                                      NULL,
                                                                                      CFSTR("!*'();:@&=+$,/?%#[]"),
                                                                                      kCFStringEncodingUTF8);
...
-(NSString *)regExWithString:(NSString *)string pattern:(NSString *)pattern{
    NSError *error = nil;
...
-(AFHTTPSessionManager*) sessionManager{
    //Create connection to sign in SS server
    NSURL *baseURL = [NSURL URLWithString:@"https://secure.nicovideo.jp"];
    AFHTTPSessionManager *manager = [[AFHTTPSessionManager alloc] initWithBaseURL:baseURL];
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    manager.requestSerializer = [AFHTTPRequestSerializer serializer];
//    [manager.requestSerializer setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];
    
    //Set connection using https
